 ...
 */
export function getTimeSlotCategory(
  timeSlotName: string
...
export function isLessonTimeSlot(timeSlot: TimeSlot): boolean {
  return (TIME_SLOT_CATEGORIES.LESSONS as readonly string[]).includes(
    timeSlot.name
  );
}
...
export function isBreakTimeSlot(timeSlot: TimeSlot): boolean {
  return (TIME_SLOT_CATEGORIES.BREAKS as readonly string[]).includes(
    timeSlot.name
  );
}
...
export function isMeetingTimeSlot(timeSlot: TimeSlot): boolean {
  return (TIME_SLOT_CATEGORIES.MEETINGS as readonly string[]).includes(
    timeSlot.name
  );
}
...
export function filterTimeSlotsByCategory(
  ...
  return timeSlots.filter(slot =>
    (categoryNames as readonly string[]).includes(slot.name)
  );
}
