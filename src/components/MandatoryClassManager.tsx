...
const MandatoryClassManager: React.FC<MandatoryClassManagerProps> = ({ onUpdate }) => {
  ...
  const columns: ColumnsType<ClassMandatoryStatus> = [
    {
      ...
      render: (text: string, record: ClassMandatoryStatus) => (
        <Space direction="vertical" size="small">
          <Text strong={record.isDirty} style={{ color: record.isDirty ? '#1890ff' : undefined }}>
            {text}
          </Text>
          {record.description && (
      ...
      render: (_, record: ClassMandatoryStatus) => (
        <Space direction="vertical" size="small">
          <Text style={{ fontSize: '12px' }}>
            {ScheduleService.formatTimeRange(record.timeSlot.startTime, record.timeSlot.endTime)}
          </Text>
    ...
    <Card 
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        <Space wrap>
          ...
          {selectedGrade && (
            <Text type="secondary">
              מתוך {filteredClasses.length} שיעורים לכיתה {ScheduleService.getGradeName(selectedGrade)}
            </Text>
          )}
        </Space>
