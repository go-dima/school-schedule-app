...
/**
 * The schedule page's tab bar: bordered card tabs above the filters bar,
 * with page actions at its end. Shared by the staff/student view tabs and the
 * per-child tabs so both read as one pattern.
 *
 * RTL: items render right-to-left in the order given (first = rightmost),
 * with the add tab last (leftmost). Inside the add tab the label comes
 * before the "+" (label rightmost). Check the on-screen order when changing.
 */
